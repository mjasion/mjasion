...
import videoLogAndLease from './1_log_and_lease.webm';
import videoMultiInstance from './2_multi_instance.webm';

> **Verified 2026-05-17.** The `coordination.k8s.io/v1` Lease API and the `client-go` leaderelection package shown here have been stable since Kubernetes 1.14. The patterns in this post apply unchanged on current Kubernetes (1.28+).
...
<Video src={videoLogAndLease} type="video/webm" preload="auto" loop="true" autoplay="true" />
...
<Video src={videoMultiInstance} type="video/webm" preload="auto" loop="true" autoplay="true" />
